...
class DefTools_Git {
	...
	public function add_toolbar_submenus( $submenus ){
		...
		$git_submenu = array();
		if ( count( $branches ) > 1 ) {
			$git_submenu = array( 'title' => __( 'Git Info', DefTools::TEXT_DOMAIN ), 'id' => 'deftools-git-info', 'href' => '#' );
		}

		foreach ($branches as $location => $branch) {
			if ( count( $branches ) > 1 ) {
				$git_title = sprintf( '%s : %s', $location, $branch );
				$git_submenu['subs'][] = array( 'title' => $git_title, 'id' => 'deftools-git-info', 'href' => '#' );
			} else {
				$git_title = sprintf( __( 'Git Info: %s:%s', DefTools::TEXT_DOMAIN ), $location, $branch );
				$git_submenu = array( 'title' => $git_title, 'id' => 'deftools-git-info', 'href' => '#' );
			}
		}
